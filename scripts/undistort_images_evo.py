...
def main():
    ...
    parser.add_argument(        
        "--indir", help="Input image directory.", default="DATADIR/flyingroom/"
    )
    args = parser.parse_args()

    print(f"Processing {args.indir}")

    evinfile_raw = os.path.join(args.indir, "evo_flyingroom.h5")
    assert os.path.isfile(evinfile_raw)
    ...
    # * copy the h5 file so that it won't be changed after this script
    ef_in_raw = h5py.File(evinfile_raw, "r+")
    evinfile = os.path.join(args.indir, 'events.h5')
    ef_in = h5py.File(evinfile, 'w')
    for ef_in_key in ef_in_raw.keys():
        ef_in.create_dataset('_' + ef_in_key, data=ef_in_raw[ef_in_key])
    ef_in_raw.close()

    ef_in['_t'][:] *=1e6 # todo:the timestamp in events.h5 is not correct. need to be fixed by shifan
    tss_evs_us = ef_in["_t"][:]
    ...
    # * trim the dataset
    first_gt_us, last_gt_us = tss_gt_us[0], tss_gt_us[-1]
    evs_first_idx = np.searchsorted(tss_evs_us, first_gt_us, side='right')
    evs_last_idx = np.searchsorted(tss_evs_us, last_gt_us, side='left')
    # print('evs', evs_first_idx, evs_last_idx)
    # assert 1>2
    ef_in_keys = ef_in.keys()
    for ef_in_key in ef_in_keys:
        ef_in.create_dataset(ef_in_key[1:], data=ef_in[ef_in_key][evs_first_idx: evs_last_idx+1])
    tss_evs_us = ef_in["t"][:]

    # img_first_idx = np.searchsorted(tss_imgs_us, first_gt_us, side='right')
    # img_last_idx = np.searchsorted(tss_imgs_us, last_gt_us, side='left')

    # * for these two lines
    # * assert tss_imgs_us[0] - (evs["t"][0]) < 1e6 and tss_imgs_us[0] - (evs["t"][0]) > 0 #todo skip for testing evo
    # * assert tss_imgs_us[-1] - (evs["t"][-1]) < 1e6 and tss_imgs_us[-1] - (evs["t"][-1]) > 0
    img_first_idx = np.searchsorted(tss_imgs_us, tss_evs_us[0], side='right') + 1
    img_last_idx = np.searchsorted(tss_imgs_us, tss_evs_us[-1], side='left') + 1
    tss_imgs_us = tss_imgs_us[img_first_idx: img_last_idx]

    if not os.path.isfile(os.path.join(args.indir, "t_offset_us.txt")) or True: # * always compute again
    ...
    # calib data
    K_rgb = np.zeros((3,3))
    K_rgb[0,0] = 197.3158535429064 ##### calib1 # *fx
    K_rgb[0,2] = 128.19252018028914 # * cx
    K_rgb[1,1] = 197.24778002324712 # * fy
    K_rgb[1,2] = 110.94875388538058 # * cy
    K_rgb[2, 2] = 1
    dist_coeffs_rgb = np.asarray([-0.3687369041657594, 0.1635331561185621, 0.0006758841262625439, -0.0014358423311193668])
    ...
    K_evs = np.zeros((3,3))
    K_evs[0,0] = 197.3158535429064 ##### calib1 # *fx
    K_evs[0,2] = 128.19252018028914 # * cx
    K_evs[1,1] = 197.24778002324712 # * fy
    K_evs[1,2] = 110.94875388538058 # * cy
    K_evs[2, 2] = 1
    dist_coeffs_evs = np.asarray([-0.3687369041657594, 0.1635331561185621, 0.0006758841262625439, -0.0014358423311193668])
    ...
    # 2) undistorting images # * also trim image folder
    print("Undistorting images")
    pbar = tqdm.tqdm(total=img_last_idx-img_first_idx+1)
    for idx, f in enumerate(img_list[img_first_idx: img_last_idx]):
        image = cv2.imread(f)
        img = cv2.undistort(image, K_rgb, dist_coeffs_rgb, newCameraMatrix=K_new_rgb)
        new_img_f = os.path.join(imgdirout, f'frame_{idx:010}.png')
        cv2.imwrite(new_img_f, img)
        print(f'{f} -> {new_img_f}')
        pbar.update(1)
